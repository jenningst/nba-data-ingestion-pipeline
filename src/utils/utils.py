...
def generate_season(year= CURRENT_YEAR):
    ...
    """
    if len(str(year)) == 4 and year != None and year != '':
        return f'{year}-{str(year + 1)[-2: len(str(year))]}'
    elif len(str(year)) == 2 and year != None and year != '':
        return f'20{year}-{str(year + 1)[-2: len(str(year))]}'
    else:
        return f'{CURRENT_YEAR}-{str(CURRENT_YEAR + 1)[-2: len(str(CURRENT_YEAR))]}'